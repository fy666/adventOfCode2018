...
fn game_result(mut player: Player, mut boss: Player, debug: bool) -> bool {
    loop {
        ...
        if debug {
            log::trace!(
                "Player deals {} damage, boss goes down to {} hit points",
                player.damage,
                boss.hit_points
            );
        }
        ...
        if debug {
            log::trace!(
                "Boss deals {} damage, player goes down to {} hit points",
                boss.damage,
                player.hit_points
            );
        }
...
pub fn run(test: bool) {
    ...
    if test {
        ...
        log::trace!(
            ...
            player,
            boss,
            game_result(player.clone(), boss.clone(), true)
        );
    } else {
            ...
            for arm in armors.iter() {
                ...
                for ring1 in rings.iter() {
                    ...
                    for ring2 in rings.iter() {
                        ...
                        if game_result(player.clone(), boss.clone(), false) {
